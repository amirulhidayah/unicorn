                    ...
                    <td>
                        <h6 class="mb-0 text-xs text-center">
                            ...
                            @if (in_array(Auth::user()->role, ['Admin', 'Operator SPM']) &&
                                ...
                                <button id="btn-upload-spm" class="btn btn-primary btn-sm mr-1 mb-1"
                                    value="{{ $data->id }}">
                    ...
                    <td>
                        <h6 class="mb-0 text-xs text-center">
                            ...
                            @if (in_array(Auth::user()->role, [
                                    ...
                                    $data->status_validasi_akhir == 1 &&
                                    $data->dokumen_spm)
                                <button id="btn-upload-sp2d" class="btn btn-primary btn-sm mr-1 mb-1"
                                    value="{{ $data->id }}">
                    ...
                    <td class="text-center">
                        @php
                            ...
                            if ($data->status_validasi_akhir == 1) {
                                $actionBtn .= '<a href="' . url('/surat-pernyataan/spp-up/' . $data->id) . '" class="btn btn-success btn-sm mr-1 mb-1"><i class="fas fa-envelope"></i> Surat Pernyataan</a>';
                            }
                            ...
                            if (($data->sekretariat_daerah_id == Auth::user()->profil->sekretariat_daerah_id && in_array(Auth::user()->role, ['Bendahara Pengeluaran', 'Bendahara Pengeluaran Pembantu', 'Bendahara Pengeluaran Pembantu Belanja Hibah'])) || Auth::user()->role == 'Admin') {
                                if (($data->status_validasi_asn == 0 && $data->status_validasi_ppk == 0) || (Auth::user()->role == 'Admin' && $data->dokumen_arsip_sp2d != null)) {
                                    $actionBtn .= '<a href="' . url('spp-up/' . $data->id . '/edit') . '" class="btn btn-primary btn-sm mb-1 mr-1"><i class="fas fa-pen"></i> Ubah</a></div>';
                                }
                                if ($data->status_validasi_asn != 0 && $data->status_validasi_ppk != 0 && ($data->status_validasi_asn == 2 || $data->status_validasi_ppk == 2)) {
                                    $actionBtn .= '<a target="_blank" href="' . Storage::url('surat_penolakan_spp_up/' . $data->surat_penolakan) . '" class="btn btn-primary btn-sm mr-1 mb-1"><i class="fas fa-file-pdf"></i> Surat Penolakan</a>';
                                    $actionBtn .= '<a href="' . url('spp-up/' . $data->id . '/edit') . '" class="btn btn-primary btn-sm mb-1 mr-1"><i class="fas fa-file-pdf"></i> Perbaiki</a></div>';
                                }
                            }

                            if (in_array(Auth::user()->role, ['ASN Sub Bagian Keuangan', 'PPK']) && $data->status_validasi_akhir == 0 && Auth::user()->is_aktif == 1) {
                                $actionBtn .= '<a class="btn btn-primary text-light btn-sm mr-1 mb-1" href="' . url('spp-up/' . $data->id) . '"><i class="far fa-check-circle"></i> Proses Validasi</a>';
                            } else {
                                $actionBtn .= '<a class="btn btn-primary text-light btn-sm mr-1 mb-1" href="' . url('spp-up/' . $data->id) . '"><i class="far fa-check-circle"></i> Lihat</a>';
                            }
                            ...
                            $actionBtn .= '<a href="' . url('spp-up/riwayat/' . $data->id) . '" class="btn btn-primary btn-sm mb-1"><i class="fas fa-history"></i> Riwayat</a>';
