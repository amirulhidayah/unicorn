        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col">
                            @livewire('dashboard.master-data.sekretariat-daerah.table')
                        </div>
                    </div>
                </div>
            </div>
...
@push('script')
    <script>
        ...
        $(document).on('click', '#btn-edit', function() {
            ...
            $.ajax({
                url: "{{ url('master-data/sekretariat-daerah') }}" + '/' + id + '/edit',
                type: "GET",
                data: {
                    id: id
                },
            ...
            swal({
                ...
                if (Update) {
                    if (aksiTambah == 'tambah') {
                        $.ajax({
                            url: "{{ url('master-data/sekretariat-daerah') }}",
                            type: 'POST',
                            data: $(this).serialize(),
                            success: function(response) {
                        ...
                        $.ajax({
                            url: "{{ url('master-data/sekretariat-daerah') }}" + '/' + idEdit,
                            type: 'PUT',
                            data: $(this).serialize(),
                            success: function(response) {
                                if (response.status == 'success') {
        ...
        $(document).on('click', '#btn-delete', function() {
            ...
            swal({
                ...
                if (Delete) {
                    $.ajax({
                        url: "{{ url('master-data/sekretariat-daerah') }}" + '/' + id,
                        type: 'DELETE',
                        data: {
                            '_token': '{{ csrf_token() }}'
                        },
