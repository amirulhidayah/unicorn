        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col">
                            @component('dashboard.components.forms.masterData.akunLainnya')
                                @slot('form_id', 'form-tambah')
                                @slot('action', url('/master-data/akun-lainnya/' . $user->id))
                                @slot('method', 'PUT')
                                @slot('back_url', url('/master-data/akun-lainnya'))
                            @endcomponent
                        </div>
                    </div>
                </div>
...
@push('script')
    <script>
        $(document).ready(function() {
            ...
            $('#aktif').val('{{ $user->is_aktif }}').trigger('change');
