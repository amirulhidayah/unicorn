             ...
             @slot('options')
                 @foreach ($daftarProgram as $program)
                     <option value="{{ $program->id }}"
                         {{ $program->id == $kegiatanSpjGu->kegiatan->program_id ? 'selected' : '' }}>
         ...
         @component('dashboard.components.formElements.select', [
             ...
             @slot('options')
                 @foreach ($daftarKegiatan as $kegiatan)
                     <option value="{{ $kegiatan->id }}" {{ $kegiatan->id == $kegiatanSpjGu->kegiatan_id ? 'selected' : '' }}>
     ...
     <td class="align-middle">
         <div style="width:150px">
             @component('dashboard.components.formElements.input', [
                 ...
                 'dataKey' => $dataKey,
                 'value' => $kegiatanSpjGu->anggaran_digunakan,
             ])
             @endcomponent
         </div>
     </td>
     <td class="align-middle" id="sisa-anggaran-{{ $dataKey }}">
         {{ number_format($jumlahAnggaran - $kegiatanSpjGu->anggaran_digunakan, 0, ',', '.') }}
     </td>
     <td class="align-middle">
         <div style="width:300px">
             @component('dashboard.components.formElements.input', [
                 'type' => 'file',
                 'id' => 'file-dokumen-' . $dataKey,
                 'name' => 'file-dokumen-' . $dataKey,
                 'class' => 'file-dokumen',
