...
if (!function_exists('jumlah_anggaran')) {
    function jumlah_anggaran($sekretariatDaerah, $kegiatan, $bulan, $tahun, $idSppLs = null)
    {
        $role = Auth::user()->role;
        ...
        $kegiatanSppLs = KegiatanSppLs::where('kegiatan_id', $kegiatan)->whereHas('sppLs', function ($query) use ($sekretariatDaerah, $bulan, $tahun, $idSppLs) {
            if ($idSppLs) {
                $query->where('id', '!=', $idSppLs);
            }
        ...
        $totalSpp = $kegiatanSppLs->sum('anggaran_digunakan');
